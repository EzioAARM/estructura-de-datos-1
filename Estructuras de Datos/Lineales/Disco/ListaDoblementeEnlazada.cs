...
{
    public class ListaDoblementeEnlazada<T> : Lista<T> where T : IComparable, ITextoTamañoFijo
    {
        #region Atributos
        ...
        private void UbicarPosicion(int posicion, ref Nodo<T> nodoAnterior, ref Nodo<T> nodoActual)
        {
            ...
            {
                nodoActual = Nodo<T>.CrearNodoDesdeDisco(_archivo, _tamañoEncabezadoBinario, _cola, _fabrica);
                for (int i = Tamanio - 1; i > posicion; i--)
                {
                    nodoActual = Nodo<T>.CrearNodoDesdeDisco(_archivo, _tamañoEncabezadoBinario, nodoActual.Anterior, _fabrica);
                }
            }
            nodoAnterior = Nodo<T>.CrearNodoDesdeDisco(_archivo, _tamañoEncabezadoBinario, nodoActual.Anterior, _fabrica);
        }

        #endregion

        public ListaDoblementeEnlazada(string nombreArchivo, IFabricaTextoTamañoFijo<T> fabrica)
        {
        ...
        public override void AgregarAlInicio(T dato)
        {
            ...
            if (_cabeza == Utilidades.ApuntadorVacio)
            {
                ...
                _cola = nodoNuevo.Posicion;
            }
            else
            {
                Nodo<T> nodoCabeza = Nodo<T>.CrearNodoDesdeDisco(_archivo, _tamañoEncabezadoBinario, _cabeza, _fabrica);
                nodoNuevo.Siguiente = _cabeza;
                nodoCabeza.Anterior = nodoNuevo.Posicion;
                _cabeza = nodoNuevo.Posicion;
        ...
        public override void Insertar(T dato, int posicion)
        {
            ...
            nodoAnterior.Siguiente = nodoNuevo.Posicion;
            nodoActual.Anterior = nodoNuevo.Posicion;
        ...
        public override void Eliminar(int posicion)
        {
            ...
            UbicarPosicion(posicion, ref nodoAnterior, ref nodoActual);
            UbicarPosicion(posicion + 1, ref nodoActual, ref nodoSiguiente);
